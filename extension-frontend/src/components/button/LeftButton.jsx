...
import { saveNoteToBackend } from "../../util/LeftButton.util"; // adjust path as needed
...
const LeftButton = ({ show }) => {
  ...
  const handleClick = async () => {
    if (showNotes && notesRef.current) {
      const noteText = notesRef.current.getCurrentNote(); // ✅ get note safely
      await saveNoteToBackend(noteText);
    }
